...
def login():
    ...
    if not check_password(password, user.password_hash):
        ...
        if user.failed_attempts >= 5:
            ...
            send_email(
                user.email,
                'CampMondo Account Locked',
                'Your account has been locked for 15 minutes after 5 failed login attempts.'
            )
        db.session.commit()
        return fail('Invalid email or password', 401)
...
def forgot_password():
    ...
    if user:
        ...
        send_email(
            user.email,
            'CampMondo Password Reset',
            f'Use this reset token within 1 hour: {raw_token}'
        )
