...
def login():
    ...
    if account_is_locked(user):
        db.session.commit()
        return fail("Account is locked or deactivated. Please try again later.", 423)
    ...
    db.session.add(AuditLog(user_id=user.id, action="LOGIN", target_table="users", target_id=user.id))
    db.session.commit()
...
def update_profile():
    ...
    if data.get("phone_number") or data.get("phone"):
        user.phone_number = data.get("phone_number") or data.get("phone")
